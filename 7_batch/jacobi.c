...
#define ROW(YY) &src[(YY)*maxXCount]
    MPI_Sendrecv(ROW(maxYCount-2), maxXCount, MPI_DOUBLE, nextRank, 0,
        ROW(0), maxXCount, MPI_DOUBLE, prevRank, 0,
        MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    MPI_Sendrecv(ROW(1), maxXCount, MPI_DOUBLE, prevRank, 0,
        ROW(maxYCount-1), maxXCount, MPI_DOUBLE, nextRank, 0,
        MPI_COMM_WORLD, MPI_STATUS_IGNORE);
}
...
int main(int argc, char **argv)
{
    ...
    // Compute each process' buffer displacement and work array size
    ...
    {
        int startRow = (rank * m) / numProcs;
        int rows = ((rank+1) * m) / numProcs - startRow;

        displs[rank] = startRow*(n+2);
        counts[rank] = rows*(n+2);
    ...
    if (myRank == 0)
        MPI_Scatterv(u_big + (n+2), counts, displs, MPI_DOUBLE,
    ...
    while (iterationCount < maxIterationCount && error > maxAcceptableError)
    {
        ...
        localError = one_jacobi_iteration(xLeft, myYBottom,
                                          ...
                                          deltaX, deltaY,
                                          alpha, relax);
        // Compute global error value
        MPI_Allreduce(&localError, &error, 1, MPI_DOUBLE, MPI_SUM,
            MPI_COMM_WORLD);
    ...
    localError = checkSolution(xLeft, myYBottom,
                               ...
                               deltaX, deltaY,
                               alpha);
    MPI_Allreduce(&localError, &absoluteError, 1, MPI_DOUBLE, MPI_SUM,
        MPI_COMM_WORLD);
    ...
    if (myRank == 0)
        MPI_Gatherv(u_old + (n+2), myRows*(n+2), MPI_DOUBLE,
                    u_big + (n+2), counts, displs, MPI_DOUBLE,
                    0, MPI_COMM_WORLD);
    else
        MPI_Gatherv(u_old + (n+2), myRows*(n+2), MPI_DOUBLE,
